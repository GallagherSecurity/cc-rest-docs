...
Finally, when you create a request, make sure its auth mode is set to 'Inherit'.  That makes it takes its authentication settings from its collection.
...
After you click Add, the domain and filename will replace the "No certificates added" line.
...
4. Sleep a short while to avoid a tight loop.
5. Stay up to date by following the `next` link.  That takes you back
   to step three.
