...
simply changes the host in its connection URL to the cloud server that the Command Centre server is
connected to.  That is most likely to be one of these two:
...
Servers 8.90 and later can write a line to an access log for every API request.

It is off by default, but you can turn it on by giving a non-zero value to a DWORD value called
`LogRESTRequests` inside the registry key `HKLM\SOFTWARE\WOW6432Node\Gallagher\Command Centre`.
Change it back to zero to stop the logging with immediate effect.

The default file name is `RESTAccess.log` and it will appear in the same folder as the server's
licence and other logs.  NLog will rotate it once per day, for ten days, then delete it.
