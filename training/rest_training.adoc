...
* Use it when REST clients cannot reach port 8904 on the server but the server can reach AWS.
* There are no licence requirements on top of the API's usual licences.  It's free.
...
* The server has to set up a cloud server and turn on the API gateway globally and in each REST Client
  item that needs it.  This is only a few clicks.
...
it (other than API keys, of course).  If a client guesses an item's ID, or even an undocumented
path, the server will enforce the licensing and privilege models.  If the session's privileges do
not permit viewing the requested item the server will respond as though the item does not exist
(with a 404).
