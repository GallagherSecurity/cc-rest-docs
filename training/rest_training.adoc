...
This is available in a link:multipage.html[multi-page] version or a link:index.html[single page] (for
easier searching and a complete table of contents).
...
A cardholder must be a member of an access group before Command Centre
will reveal his or her personal data (see next).
...
Importantly, PDFs are attached to access groups. Clients can see a cardholder's
...
just like direct members).  The API will let you set a cardholder's
PDF value even if they are not a member of the correct group, but it
would be unwise to rely on that behaviour.
...
Enable the REST API and–-for the moment–-tick the checkbox to the right of the port. This is a recent
...
WARNING:  ⚠ *Make sure 'Enable REST Clients with no client certificate' is off in production*. In 8.50 it
changed its name from 'Do not require pinned client certificates'. It ships turned off: make sure
...
eventually choose to do it, it all happens under a button
labelled 'Manage Certificates'. The Configuration Client’s online help
...
=== Install the DevHeader extension
...
WARNING:  ⚠ *The ModHeader extension contains malware.  Do not use it.*

Install DevHeader into Chrome or Brave and click on the 'DevHeader' icon.  Under "Header Rules" set
a header called `Authorization` with a value of `GGL-API-KEY` followed by a space and the API key you
took from the configuration client. Note in the example below I have two headers ready to go, only
one of which is active.

Both the `GGL-API-KEY` and the letters in the 30+ characters that follow it must be upper case.

image::devheader_setup_1.png[DevHeader browser extension setup,title="DevHeader browser extension setup"]

WARNING:  ⚠ *Set a domain so that the header only goes to your Command Centre
server.  Otherwise DevHeader will send your API key to every site you visit*.

Use a URL pattern in the filter that all your API queries will match but
other web browsing will not.

image::devheader_setup_2.png[Limiting DevHeader's reach,title="Limiting DevHeader's reach"]
...
If you want to do more than look, you need an application such as Postman or Bruno.
...
Bruno is an alternative to PostMan.  You do not need both.

I suggest creating a collection to hold all your requests, then setting common configuration on the
collection rather than on each request.  The instructions below expect that.

=== Send the API header with every request

This is what the Modify Headers extension does in Chrome. It makes Bruno send an Authorization
header containing your API key with its request.

After creating a collection, edit its settings and go to its Auth tab:

image::bruno_auth_header.png[Setting auth header in Bruno,title="Setting auth header in Bruno"]

That screenshot shows the value of the API key being `{{apiKey}}`, which is a variable set in what
Bruno calls the _environment_, which is another collection of settings you choose from a drop-down
in the top right corner of the Bruno window.

If that is too much redirection for you, replace the `{{apiKey}}` with your API key (do not include
any curly braces).  But if you want to use environments, they are a great way to quickly switch
between Command Centre operator settings.

image::bruno_environment.png[Setting apiKey var in Bruno env,title="Setting apiKey var in Bruno env"]

NOTE: The variable name is case-sensitive.  `apikey`, all in lower case, may be a better choice than
that shown in the screenshots.

Finally, when you create a request, make sure its auth mode is set to 'Inherit'.  That makes it take
its authentication settings from its collection.

image::bruno_request_auth.png[Bruno request inheriting auth,title="Bruno request inheriting auth"]

=== Never mind that your server certificate is self-signed

Bruno's settings cog is in the bottom-left corner.  Turn off "SSL/TLS Certificate Validation".

image::bruno_server_cert_warning_off.png[Bruno SSL cert verification off,title="Bruno SSL cert verification off"]


=== Set the content type to JSON

When you create a POST or PATCH you will have to set its type before you can edit the body.  Be sure
to pick JSON.

image::bruno_content_type.png[Content-type Bruno content type,title="Bruno content type"]
...
Controller names are plural, as a rule, and pothole_cased. You can find links
to them all with
...
properties (Web Services tab), or if you are running 8.50 or better and the REST
...
....
GET https://localhost:8904/api/events?cardholder=325,8445
....
...
If you are running Windows and entered that command into a Unix-like shell, it may have translated
the `/CN=RESTtest`` into `C:/Program Files/.../CN=RESTtest` which would have caused `openssl req` to
complain about the name.  You can defeat the Unix-to-Windows path conversion by adding
`MSYS_NO_PATHCONV=1` to the head of your command line.

The certificate's name will be 'RESTtest'.  You should come up with a name related to your
application's purpose because diagnosing certificate problems is a tough enough job without
ambiguous certificate names muddying the waters even more.
...
Now you need to add the certificate to the clients that need it. If you use Postman or Bruno
...
At time of writing, the standalone version of Postman cannot read certificates out of the
...
Look in the Client Certificates tab in your collection's settings.  If you followed the instructions
above for creating a client certificate, both the certificate file and key file will be the same.

If you protected your private key with a password (a good idea, but turned off by `-nodes`), give it
to Bruno here.

image::bruno_client_cert.png[Tell Bruno which client cert to use,title="Tell Bruno which client cert to use"]

After you click Add, the domain and filename will replace the "No client certificates added" line.
...
=== Subscribing to updates (one item at a time, pre-8.30)

You really should be using bulk item updates, covered in the section
below, because monitoring one item at a time requires one TCP
connection per item. So why use this?

* You are not running 8.30 yet, or
* you are monitoring only one item and want a slightly simpler call, or
* your client wants to wait longer than 30s between GETs (but it is hard
  to imagine a client needing that).

Because those are all extremely unlikely, this section does not cover
the process in great detail.  But this is it:

1. GET the item's details page.
2. Follow the link called `updates` on that page.
3. It is a long poll, so the server won’t respond until it has
   something for you (or it times out after about 50s).
4. Process the results.
5. Sleep a short while to avoid a tight loop.
6. Stay up to date by following the `next` link.  That takes you back
   to step three.


=== Subscribing to updates (many items, 8.30+)

Here is your process:

1. GET `/api`, as ever.
2. Use the API to get the IDs of the items you want to monitor.
3. POST a document to the link at `features.items.updates` (which is
   `/api/items/updates` in 8.30, but may change, which is why you
   should use the page at `/api`).  The body of the POST should
   contain the IDs of your items.  There is an example below.
4. The POST will return with the status of all your items and a `next`
   link.
5. Process the results.
6. Sleep briefly to avoid tight loops.  But not longer than 30s,
   otherwise the server will drop your session, thinking you have
   walked away.
7. GET the `next` link.
8. Go to step five.
...
must be in quotes because in the future they could contain alphas.
Place as many in the array as you like.  We tested 1,000 without it
affecting performance.

This is JSON, so all the whitespace is optional: omit it if you have
thousands of items.

In early versions of Command Centre the first GET will return the same
states that the POST did, which seems redundant, but that is just the
way it was. Just keep looping.

This is a far better way of monitoring multiple items that the
previous because it only uses one server connection per client.

At time of writing (9.40) this is the only API call that maintains
state between calls.  All the others require their context to arrive
with the query.
...
=== Connecting your server to the cloud gateway

1. In the Configuration Client, pick 'Services and Workstations' from the Configure menu.

2. Right-click in the window that appears, then choose New → Command Centre Cloud.

3. Name the item in the General tab.  This does not need to be too descriptive since each site only
   has the one.

4. Go to the Configuration tab and tick 'Enable Gallagher API Gateway', then select the region
   closest to your server.

5. You also need to set "Default country code for SMS Verification" even though it will only be used
   by those working with the Mobile Connect SDK.

Apply or save the item.

You can now check the 'Status and Overrides' tab to see if Command Centre has connected to the
cloud.  Expect 'Receive: Online' and 'Send: Idle'.

image::cloud_item.png[Connecting Command Centre to the Gallagher API gateway,title="Connecting to the cloud API gateway"]

=== Enabling cloud clients to reach REST Client items

To limit the exposure of REST Client items, each determines its own visibility through the public IP
addresses in the cloud.  All are turned off by default.

To turn it on, first open your REST Client items in the Configuration Client's 'Configure' menu in
the 'Services and Workstations' item.

Then open the REST Client(s) that you want to be reachable via the cloud and turn on 'Allow
connection via the Gallagher API Gateway' in the Connections tab.

image::rest_client_gateway.png[Allowing a client through the gateway,title="Enabling the gateway per client"]
...
|View encrypted cardholder user codes
|'View Encrypted Cardholder User Codes'.  'Advanced User' does not include this privilege.
...
Access Control' on the group’s division.
...
|Create or edit access groups (except memberships)
|'Edit Access Groups' on the access group’s division.
...
* access groups (read-write in 9.30),
* competencies,
* card types,
* roles,
* lockers,
