...
1. The table of contents is generated by github pages (Jekyll, Kramdown, Maruku).  If you're seeing
   this, it's because you're reading the README.md on github.com instead of
   https://gallaghersecurity.github.io/cc-rest-docs/training .  In that case, there is a table of
   contents button above and to the left of this text.
{:toc}
<!-- and that {:toc} has to be on its own line -->

<!-- {::options toc_levels="2" syntax_highlighter_opts="{line_numbers: true}" } -->

<!-- A manual (and out-of-date) ToC follows, should we ever need to reinstate one.
...
> trust the server, Command Centre cannot raise any alarms, because it never received a proper
...
fiddly if the server is checking client certificates).
