...
Take a copy of your version of the first URL, the one marked <1>. It is the href of the cardholder
group membership, which is a link between the cardholder (ID 325, in my case) and the access group
(ID 5338). It starts with the href of the cardholder, because it is a property of that cardholder
and serviced by the cardholders controller, but do not try to interpret it more. Certainly do not
read anything into the number on the end (1069), and do not be surprised if you have an item with
the same ID. Sections 16.3 and 16.4 go into what you should not do with hrefs.
...
Never mind the competency marked <2> yet. The `zzz` makes the server ignore it.

First you will need to change the numbers marked <1> to the IDs of an access group and a card type
on your system. You can get those with:
...
might like to stop the client from checking it as you told Chrome and Postman to do earlier.
...
. wait, then
. use the signed certificate on your web server.
...
. Go to Server properties, Web Services, REST API section Manage Certificates,
