...
In production environments, clients should refuse to talk to a server that offers a certificate that
is not carrying a signature from one of the internet’s signing authorities. One that has not been
paid for, in other words.
...
If you will pardon a very brief dive into source code, here is one way of skipping server
certificate checking in a C# client:
...
(which is advisable), here are a couple of approaches.
